...
function faFindTopWishlistLink(root, maxTop) {
    var links = root && root.querySelectorAll ? root.querySelectorAll('a[href*="wishlist"]') : [];
    var best = null;
    var bestTop = Infinity;
    for (var i = 0; i < links.length; i++) {
        var text = String(links[i].textContent || '').replace(/\s+/g, ' ').trim();
        if (!/(?:愿望单|wishlist)/i.test(text) || text.length > 24) continue;
        var top = Infinity;
        try {
            var rect = links[i].getBoundingClientRect();
            top = rect && Number.isFinite(rect.top) ? rect.top : Infinity;
        } catch (_) {}
        if (links[i].closest && links[i].closest('header')) top = Math.min(top, 0);
