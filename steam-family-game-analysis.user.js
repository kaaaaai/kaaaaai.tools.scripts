...
// @version      2.06
// @description  扫描 Steam 家庭库库存，在游戏页面标记已有游戏，并提供家庭库数据分析。v2.06: 识别 Steam App 非链接式 React 顶栏入口。
// @author       SmallRob
...
// <FA_TOP_NAV_HELPERS>
function faFindTopWishlistLink(root, maxTop) {
    var elements = root && root.querySelectorAll ? root.querySelectorAll('a[href*="wishlist"],button,[role="button"],[tabindex],span,div') : [];
    var best = null;
    var bestTop = Infinity;
    var bestPriority = -1;
    for (var i = 0; i < elements.length; i++) {
        var text = String(elements[i].textContent || '').replace(/\s+/g, ' ').trim();
        if (!/^(?:愿望单|wishlist)(?:\s*\d+)?$/i.test(text)) continue;
        var control = (elements[i].closest && elements[i].closest('a,button,[role="button"],[tabindex]')) || elements[i];
        if (!control) continue;
        var priority = control.matches && control.matches('a,button,[role="button"],[tabindex]') ? 2 : 1;
        var top = Infinity;
        try {
            var rect = control.getBoundingClientRect();
            top = rect && Number.isFinite(rect.top) ? rect.top : Infinity;
        } catch (_) {}
        if (control.closest && control.closest('header')) top = Math.min(top, 0);
        if (top >= 0 && top <= maxTop && (priority > bestPriority || (priority === bestPriority && top < bestTop))) {
            best = control;
            bestTop = top;
...
function faFindTopSteamLogoLink(root, maxTop) {
    ...
    for (var i = 0; i < candidates.length; i++) {
        ...
        var control = String(candidate.tagName || '').toLowerCase() === 'img'
            ? ((candidate.closest && candidate.closest('a,button,[role="button"]')) || candidate.parentElement)
            : candidate;
        if (!control || control === best) continue;
        var tag = String(control.tagName || '').toLowerCase();
        var href = String(control.getAttribute && control.getAttribute('href') || '');
        if (tag === 'a' && !/^(?:\/|https:\/\/store\.steampowered\.com\/)(?:[?#].*)?$/.test(href)) continue;
        var image = tag === 'img' ? control : (control.querySelector && control.querySelector('img[alt],img[src]'));
        var imageHint = image ? String(image.getAttribute('alt') || image.getAttribute('src') || '') : '';
        var label = String(control.getAttribute && control.getAttribute('aria-label') || control.textContent || '').replace(/\s+/g, ' ').trim();
        if (!/(?:steam|header_logo)/i.test(imageHint) && !/^steam$/i.test(label)) continue;
        var top = Infinity;
        try {
            var rect = control.getBoundingClientRect();
            top = rect && Number.isFinite(rect.top) ? rect.top : Infinity;
        } catch (_) {}
        if (top >= 0 && top <= maxTop && top < bestTop) {
            best = control;
            bestTop = top;
        }
    }
...
function faReportTopNavigationReady() {
    try {
        if (window.__FA_QX__ && typeof window.__FA_QX__.reportNavigation === 'function') {
...
(function() {
    ...
    function init(){
        ...
        faStyleTopNavigationEntry(setting_btn, 'inline');
        ...
        function plug(){
            ...
            if(headding){
                ...
                faReportTopNavigationReady();
                return;
            }
            // v2.05: Steam App 的 React 顶栏没有语义 header/aria-label。
            // 优先插入可见的顶部愿望单 tab；紧凑子页面没有愿望单时，退到顶部 Steam Logo 右侧。
            var topWishlist = faFindTopWishlistLink(document, 420);
            if(topWishlist && topWishlist.parentNode){
                var placement = faFindTopNavigationPlacement(topWishlist, 420);
                if(placement){
                    setting_btn.className = placement.before.className || topWishlist.className || 'Focusable';
                    faStyleTopNavigationEntry(setting_btn, 'inline');
                    placement.row.insertBefore(setting_btn, placement.before);
                }else{
                    setting_btn.className = topWishlist.className || 'Focusable';
                    faStyleTopNavigationEntry(setting_btn, 'fallback');
                    topWishlist.parentNode.insertBefore(setting_btn, topWishlist);
                }
                faReportTopNavigationReady();
                return;
            }
            var topLogo = faFindTopSteamLogoLink(document, 320);
            if(topLogo && topLogo.parentNode){
                faStyleTopNavigationEntry(setting_btn, 'fallback');
                topLogo.parentNode.insertBefore(setting_btn, topLogo.nextSibling);
                faReportTopNavigationReady();
                return;
            }
            setTimeout(plug,200)
        ...
        function plugWishlistSibling(){
            ...
            var wlLink = faFindTopWishlistLink(document, 420);
            if(!wlLink){
