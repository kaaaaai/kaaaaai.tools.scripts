...
const releaseDir = path.join(root, 'quantumultx/steam-family/releases/0.2.4');
const sha256 = (text) => crypto.createHash('sha256').update(text).digest('hex');

test('full-core release publishes the v2.08 QX contract and required assets', () => {
  const metadata = JSON.parse(fs.readFileSync(path.join(root, 'src/quantumultx/steam-family/release.json'), 'utf8'));
  assert.equal(metadata.release, '0.2.4');
  assert.equal(metadata.coreVersion, '2.08');
  ...
  const fullReleaseDir = path.join(root, 'quantumultx/steam-family/releases/0.2.4');
...
function snapshotPublishedFiles(temporaryRoot) {
  ...
  const releaseRoot = path.join(publishedRoot, 'releases/0.2.4');
...
test('build emits a self-consistent release and stable snippets', () => {
  ...
  assert.equal(manifest.release, '0.2.4');
  assert.equal(manifest.coreVersion, '2.08');
  ...
  assert.match(canonical, /releases\/0\.2\.4\/injector\.js/);
  assert.match(canonical, /script-echo-response .*releases\/0\.2\.4\/runtime-asset\.js/);
  assert.match(canonical, /script-echo-response .*releases\/0\.2\.4\/bridge\.js/);
});
