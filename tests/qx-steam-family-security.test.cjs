...
test('bridge exposes exactly the six Phase 1 operations and no network credential primitives', () => {
  for (const file of [path.join(sourceDir, 'bridge.js'), path.join(runtimeDir, 'releases/0.2.0/bridge.js')]) {
    const text = fs.readFileSync(file, 'utf8');
...
test('release metadata generates one exact handler entry for every Phase 1 operation', () => {
  ...
  const generated = fs.readFileSync(path.join(runtimeDir, 'releases/0.2.0/bridge.js'), 'utf8');
