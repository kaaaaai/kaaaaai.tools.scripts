...
test('asset service maps virtual names to fixed pinned sources', async () => {
  const chart = await call('https://store.steampowered.com/fa-qx/v1/asset/chart.js?release=0.2.2');
  ...
  assert.equal(core.requests[0].url, 'https://raw.githubusercontent.com/kaaaaai/kaaaaai.tools.scripts/main/quantumultx/steam-family/releases/0.2.2/core.js');
});
